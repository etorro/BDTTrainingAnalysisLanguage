...
def process_ast_node(visitor, gc, call_node):
    ...
    for arg,dest in zip(cpp_ast_node.args, call_node.args):
        print("in cpp_ast process, arg: ", arg, " dest: ", dest)
        rep = visitor.get_rep(dest)
        print("in cpp_ast process, rep: ", rep)
        print("in cpp_ast process, rep type: ", type(rep))
        if type(rep) == str:
            print("it is str: in cpp_ast process, rep type: ", type(rep))
